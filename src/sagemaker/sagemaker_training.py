...
from src.utils.project_dirs import get_src_dir, dataset_splits_dir, project_root
import tempfile
import os
...
def run_sagemaker_training(entry_point, dataset, parameters, config_name, num_days=14):
    ...
    sagemaker_dir = Path(tempfile.mkdtemp(prefix=f"countnet_sagemaker_{dataset}_{Path(entry_point).stem}_"))
...
def process_config(config_name, training_config):
    ...
    for hp in combinations(hyperparameters):
        ...
        if is_possible(all_params, impossible_combinations):
            ...
            full_config_name = config_name + "_" + dataset + "-" + job_suffix
