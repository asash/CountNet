...
import json
from src.recommenders.SASRecCountNet import SASRecCountNetRecommender, SASRecCountNetConfig
import dill
...
parameters = get_training_parameters((("dataset", str, "booking"),
                                      ("device", str, "cuda:0"),
                                      ("sandbox-training", bool, True),
                                      ("eval", bool, True),
                                      ("max-batches-per-epoch", int, 128),
                                      ("max-epochs", int, 10000),
                                      ("batch-size", int, 512),
                                      ("sequence-length", int, 200),
                                      ("effective-batch-size", int, 512),
                                      ("count-transform", str, "id"),
                                      ("dropout", float, 0.5),
                                      ("rerank-cutoffs", str, ""),
                                      ("logit-aggregate", str, "replace"),
                                      ("logit-item-repr", str, "embeddings"),
                                      ("separate-logit-projection", bool, False),
                                      ("backbone", str,"SASRec"),
                                      ("beta", float, "1.0")
                                      ))
...
config = SASRecCountNetConfig(device=parameters['device'], batches_per_epoch=parameters['max_batches_per_epoch'],
                              max_epoch=parameters["max_epochs"],
                              batch_size =parameters["batch_size"],
                              effective_batch_size =parameters["effective_batch_size"],
                              count_transform=parameters["count_transform"],
                              dropout=parameters["dropout"],
                              logit_aggregate=parameters["logit_aggregate"],
                              sequence_len=parameters["sequence_length"],
                              logit_item_repr=parameters["logit_item_repr"],
                              separate_logit_projection=parameters["separate_logit_projection"],
                              backbone=parameters["backbone"],
                              rerank_cutoffs=parameters["rerank_cutoffs"],
...
def train():
    recommender = SASRecCountNetRecommender(config)
...
if parameters["sandbox_training"]:
